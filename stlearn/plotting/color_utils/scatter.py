...
import matplotlib.pyplot as plt
import numpy as np
from stlearn.plotting.color_utils import Colorizer
...
def scatterc(
		...
		*, 
		...
		colors = None, 
		labels = None, 
		legend: Optional[str] = "outside", 
		g: np.ndarray = None, 
		gcolor: str = "thistle", 
		galpha: float = 0.1, 
		glinewidths: float = 0.25, 
		**kwargs) -> None:
	if colors is None:
...
def scattern(
	...
	if bgval is not None:
		...
		ax.scatter(xy[:, 0], xy[:, 1], c=color, s=s, lw=lw, cmap=cmap, **kwargs)
	if g is not None:
		ax = plt.gca()
		_draw_edges(ax, xy, g, gcolor, galpha, glinewidths)
